...
export function NoteList({ layer = 0, parentId }: NoteListProps) {
  ...
  const fetchChildren = async(e:React.MouseEvent,note:Note) => {
    ...
    setExpanded((prev) => {
      const newExpandend = new Map(prev);
      newExpandend.set(note.id, !prev.get(note.id));
      return newExpandend;
    })
  ...
  return (
    <>
      ...
      .map((note) => {
        return (
          <div key={note.id}>
            <NoteItem note={note} layer={layer} expanded={expanded.get(note.id)} onExpand={(e:React.MouseEvent) => fetchChildren(e,note)} onCreate={(e) => createChild(e,note.id)}/>
